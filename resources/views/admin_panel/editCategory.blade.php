...
<body>
    ...
    <form id="insertProduct" action="/category/updateStore/{{ $category->id }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="block w-11/12 p-6 mx-auto mt-6 bg-white rounded-lg shadow-lg">
            <h1 class="mb-4 text-xl text-blue-700 uppercase">Update Category</h1>
            <div class="mb-6 form-group">
                <label for="product_name" class="inline-block mb-2 text-gray-700 form-label">Category Name</label>
                <input type="text"
              ...
              focus:text-gray-700 focus:bg-white focus:border-blue-700 focus:outline-none"
                    id="product_name" placeholder="Product Name" name="name" value="{{ $category->name }}">
                @error('name')
            ...
            <div class="flex mb-6 form-group">
                <div class="flex justify-center">
                  ...
                  <img class="mt-4" src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" style="height:40vh;">
                  @error('image')
    <p class="pt-1 pl-1 text-sm text-red-600">{{ $message }}</p>
@enderror
                </div>
