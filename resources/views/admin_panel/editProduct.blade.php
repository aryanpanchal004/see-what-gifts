...
<body>
    ...
    <form id="insertProduct" action="/product/update/{{ $product->id }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="block w-11/12 p-6 mx-auto mt-6 bg-white rounded-lg shadow-lg">
            <h1 class="mb-4 text-xl text-blue-700 uppercase">Update Product</h1>
            <div class="mb-6 form-group">
              ...
              focus:text-gray-700 focus:bg-white focus:border-blue-700 focus:outline-none"
                    id="product_name" placeholder="Product Name" name="name" value="{{ $product->name }}">
                @error('name')
            ...
            <div class="flex mb-6 form-group">
                <div class="flex justify-center">
                  ...
                  <img class="mt-4" src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" style="height:40vh;">
                  @error('image')
    <p class="pt-1 pl-1 text-sm text-red-600">{{ $message }}</p>
@enderror
                </div>
          ...
          <div class="mb-6 form-group">
            ...
            <div class="mb-6 form-group">
              ...
              focus:text-gray-700 focus:bg-white focus:border-blue-700 focus:outline-none
            "
                    id="product_description" rows="3" placeholder="Product Description" name="desc">{{ $product->desc }}</textarea>
                @error('desc')
                    <p class="pt-1 pl-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-6 form-group">
                <div class="flex">
                  ...
                  m-0
                  focus:text-gray-700 focus:bg-white focus:border-blue-700 focus:outline-none"
                            aria-label="Default select example" placeholder="499.00" value="{{ $product->price }}">
                        @error('price')
                            <p class="pt-1 pl-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
